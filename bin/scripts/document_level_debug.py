...
It's sole purpose is to create a debug version of the output.
construct_event_database already has a debug option,
but it's output is not the same as this one. This one
is more comprehensive.
"""

# TODO: Either merge this debug version into the original script (This
# may hurt the speed of the original script). Or, delete everything
# debug related from the original script, and use this
# for debug purposes (When we update the original, we must update this
# file as well).
...
if __name__ == "__main__":
    ...
    for json_data in input_file:
        ...
        debug_data["violent"] = "violent" if json_data["is_violent"] == 1 else "non-violent"
        debug_data["urbanrural"] = json_data["urbanrural"]
        # debug_data["title"] = json_data.get("title", "")
        # debug_data["year"] = int(json_data["html_year"])
        # debug_data["month"] = int(json_data["html_month"])
        # debug_data["day"] = int(json_data["html_day"])
        ...
        for cluster in clusters:
            ...
            out_json["sentence_ids"] = cluster
            out_json["extracted_places"] = list(all_place.keys())
            out_json["is_kept"] = "Kept"
            ...
            if is_html_foreign:
                ...
                out_json["geocoding_status"] = "Fail - Foreign Html Place"
            ...
            if args["check_extracted_first"] or html_latitude == 0.0: # if html place failed or was non-existent (If check_extracted_places_first is True enters here regardless!)
                ...
                if any([is_foreign_country(place_name) for place_name in all_place]):
                    ...
                    out_json["is_kept"] = "Discarded"
                    out_json["geocoding_status"] = "Fail - Foreign Extracted Place"
                    debug_clusters.append(out_json)
                ...
                if args["dist_has_locality"]: # dist_dict has "locality" key
                    ...
                    if len(possible_places) > 0: # we had at least one non-error
                        ...
                        if vals[0] == "geopy":
                            ...
                            out_json["geocoding_status"] = "Success - Geopy"
                            debug_clusters.append(out_json)

                        else:
                            latitude, longitude, curr_place_name, curr_state_name = vals
                            out_json["geocoding_status"] = "Success - Dictionary"
                            debug_clusters.append(out_json)
                ...
                if latitude == 0.0:
                    ...
                    if args["check_extracted_first"] and html_latitude != 0.0:
                        ...
                        out_json["geocoding_status"] = "Success - From Html Place"
                        debug_clusters.append(out_json)
                        ...
                        if state_alts.get(html_place, "") != "": # Check for state name in html place
                            ...
                            for place_name in all_place.most_common():
                                if state_alts.get(place_name[0], "") != "":
                                    ...
                                    out_json["geocoding_status"] = "Only State - From Extracted Places"
                                    debug_clusters.append(out_json)
